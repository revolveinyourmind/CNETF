...
if(date != history[dim(history[,1])[1],1]){
  close <- c()
  for(i in code[1:17]){
...
for(i in 2:19){
  rate <- c()
  for(j in 0:119){
    r <- round(((as.numeric(history[dim(history[,1])[1]-j, i]) - as.numeric(history[dim(history[,1])[1]-j-120, i]))/as.numeric(history[dim(history[,1])[1]-j-120, i]))*100, 2)
